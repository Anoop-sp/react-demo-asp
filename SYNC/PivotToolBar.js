...
import jsonData from "./pivot-data/universitydata.json";
import { select, createElement } from "@syncfusion/ej2-base";
// import "./overview.css";
/**
 * PivotView Toolbar Sample
 */
let UniversityData = jsonData.data;
let dataSourceSettings = {
  ...
  //   filterSettings: [
  //     { name: "region", type: "Exclude", items: ["Africa", "Latin America"] },
  //   ],
  //   fieldMapping: [
...
function PivotToolbar() {
  ...
  function queryCellInfo(args) {
    if (
      args.cell &&
      args.cell.classList.contains("e-valuescontent") &&
      args.data &&
      args.data[0].hasChild
    ) {
      let pivotValues;
      let colIndex = Number(args.cell.getAttribute("data-colindex"));
      if (!isNaN(colIndex)) {
        pivotValues =
          pivotObj.pivotValues[args.data[colIndex].rowIndex][
            args.data[colIndex].colIndex
          ];
      }
      if (
        pivotValues &&
        args.cell &&
        args.cell.classList.contains(pivotValues.cssClass)
      ) {
